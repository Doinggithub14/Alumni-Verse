  ...
  const [collabs] = useState([
    {
      ...
      description: "Looking for a frontend dev and ML engineer to join this precision agriculture startup idea.",
    ...
    },
    {
      ...
      description: "Need data analysts and researchers to collaborate on UN-funded climate research.",
    ...
    },
    {
      ...
      description: "Seeking UI/UX designers and content creators passionate about mental wellness.",
  ...
  return (
    <div className="bg-black text-white min-h-screen mt-10 py-16 px-6">
      ...
      <p className="text-center text-neutral-300 max-w-xl mx-auto mb-10">
        Discover and join exciting alumni-led projects, or pitch your own and find passionate collaborators from our global alumni network.
      </p>

      {/* Featured Collaborations */}
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8 max-w-6xl mx-auto mb-20">
        {collabs.map(c => (
          <div key={c.id} className="bg-gray-900 border border-purple-700 rounded-xl p-6 hover:shadow-lg transition">
            <h3 className="text-xl font-bold text-purple-300 mb-1">{c.title}</h3>
            <p className="text-sm text-gray-400 mb-3 italic">{c.owner}</p>
            <p className="text-gray-300 mb-4 text-sm">{c.description}</p>
            <div className="flex flex-wrap gap-2">
              {c.tags.map(tag => (
                <span key={tag} className="bg-purple-700 text-xs text-white px-3 py-1 rounded-full">
                  #{tag}
                </span>
              ))}
            </div>
      ...
      <section className="bg-gradient-to-r from-purple-950 to-black py-12 px-6 rounded-xl max-w-4xl mx-auto border border-purple-700">
        <h2 className="text-3xl font-bold text-purple-300 mb-4 text-center">🚀 Submit Your Collaboration Idea</h2>
        <p className="text-center text-gray-400 mb-8 text-sm">
          Got an idea? Share it here and we'll feature it for others to discover.
        </p>
